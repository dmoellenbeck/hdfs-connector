...
public class HdfsOperations {
	...
	/**
	 * Read the content of a file designated by its path and streams it to the
	 * rest of the flow:
	 * 
	 * @param connection
	...
	/**
	 * Write the current payload to the designated path, either creating a new
	...
	public List<FileStatus> listStatus(@Connection HdfsConnection connection, final String path,
		...
		try {
			...
			return flsDtos.stream().map(flsDto -> beanMapper.map(flsDto, FileStatus.class))
	 ...
	 * <b>Note:</b> by default the Hadoop server has the append option disabled.
	 * In order to be able append any data to an existing file refer to
	 ...
	 * <li>hdfs.file.checksum - MD5 digest of the file (if it is a file and
	 * exists)</li>
	 * <li>hdfs.file.status - A Hadoop object that contains info about the
	 * status of the file (org.apache.hadoop.fs.FileStatus</li>
	 * </ul>
	 *
	 * @param path
	 ...
	 * useRawLocalFileSystem indicates whether to use RawLocalFileSystem as it
	 * is a non CRC File System.
	 * 
	 * @param connection
	 *            the connection object
	 * @param deleteSource
	...
	/**
	 * Copy the source file on the local disk to the FileSystem at the given
	 * target path, set deleteSource if the source should be removed.
	 * 
	 * 
	 * @param connection
	 *            the connection object
	...
	/**
	 * Set owner of a path (i.e., a file or a directory). The parameters
