...
public class HdfsOperations extends BaseOperations {
    ...
    public HdfsOperations(ServiceFactory serviceFactory) {
...

    {
        ...
        return execute(() -> Result.<InputStream, Void>builder().mediaType(MediaType.BINARY).output(hdfsApiService.read(path, bufferSize)).build());
    ...
    public List<FileStatus> globStatus(@Connection HdfsConnection connection, String pathPattern,
        ...
        HdfsAPIService hdfsApiService = serviceFactory.getService(connection);
        return execute(() -> { List<FileStatusDTO> flsDtos = hdfsApiService.globStatus(pathPattern, filter);
            BeanMapper beanMapper = dozerMapper();
            return flsDtos.stream()
                    .map(flsDto -> beanMapper.map(flsDto, FileStatus.class))
                    .collect(Collectors.toList());});
    ...
    public MetaData getMetadata(@Connection HdfsConnection connection, final String path) {
        ...
        HdfsAPIService hdfsApiService = serviceFactory.getService(connection);
           return execute(()->dozerMapper().map(hdfsApiService.getMetadata(path),MetaData.class));
    }
